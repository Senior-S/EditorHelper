...
public class FoliageManager
{
    ...
    public void CustomUpdate(FoliageEditor foliageEditorInstance)
    {
        ...
        foliageEditorInstance.isPointerOnWorld = Physics.Raycast(ray, out var hitInfo, 8192f, (int)DevkitFoliageToolOptions.instance.surfaceMask);
				...
				for (int i = foliageBounds.min.x; i <= foliageBounds.max.x; i++)
				{
					for (int j = foliageBounds.min.y; j <= foliageBounds.max.y; j++)
					{
						...
						if (key)
						{
							...
							{
								if (tile.instances.TryGetValue(foliageEditorInstance.selectedInstanceAsset.getReferenceTo<FoliageInstancedMeshInfoAsset>(), out var value))
								{
					...
					foreach (Vector2Int item in Regions.GetCoordinateBoundsInt(worldBounds))
					{
						...
						if (treesOrNullInRegion != null)
						{
							for (int num4 = treesOrNullInRegion.Count - 1; num4 >= 0; num4--)
							{
								...
								if (sqrMagnitude < num)
								{
									...
									foliageEditorInstance.previewSamples.Add(new FoliagePreviewSample(resourceSpawnpoint.point, flag3 ? Color.red : (Color.red / 2f)));
						...
						for (int num5 = list.Count - 1; num5 >= 0; num5--)
						{
							...
							if (sqrMagnitude2 < num)
							{
								...
								foliageEditorInstance.previewSamples.Add(new FoliagePreviewSample(levelObject.transform.position, flag6 ? Color.red : (Color.red / 2f)));
