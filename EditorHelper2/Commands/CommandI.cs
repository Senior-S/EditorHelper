...
namespace EditorHelper2.Commands;
...
public class CommandI : Command
{
    protected override void execute(CSteamID executorID, string parameter)
    {
        ...
        {
            if (SDG.Unturned.Assets.find(guid) is ItemAsset itemAsset)
            {
                GiveItem(itemAsset);
            }
        ...
        {
            if (SDG.Unturned.Assets.find(EAssetType.ITEM, id) is ItemAsset itemAsset)
            {
                GiveItem(itemAsset);
            }
